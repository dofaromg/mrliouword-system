...
- **檔案不要亂刪，留存紀錄。**（擁有者指示）
- **任何刪除都要反覆確認；先暫停或關閉即可；不允許刪除任何歷史。**
  （擁有者指示，2026-09-22：「不要亂刪我東西。任何刪除都要反覆確認，然後先暫停
  關閉即可，我不允許刪除任何歷史，要查清楚確認。」）
  範圍包含**暫存檔、測試副本、憑證檔、分支、trigger、對話串**——不只倉庫檔案。
  刪除前：查清楚是什麼、列出來、問一次、再問一次；能用「暫停／關閉／隔離
  （改名、`chmod 000`、移到 paused 目錄）」達到目的的，一律不刪。
  背景：2026-09-22 session 裡曾以 `shred -u` 銷毀擁有者提供的 token 暫存檔、
  以 `rm -rf` 清掉兩個測試副本，均未先問。見該日紀錄第十二節。
...
1. **在指令執行前就寫下結果**（2026-09-22 列入）。計數、SHA、狀態、
   「應該是 X」、**commit message 裡的統計與「零刪除」之類的模板句**——
   任何數字與狀態只能從指令輸出複製，不能從預期填入，不能從模板填入。
