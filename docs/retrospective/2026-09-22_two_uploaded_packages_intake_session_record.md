...
| 5 | **上一列的數字是錯的，而且是在寫「照抄數字」那一則時錯的**：我寫「磁碟上 42 檔、`git ls-files` 只有 39 檔」，但那條指令的實際輸出是 `43` 與 `40`（`packages/README.md` 也算一檔）。42 與 39 是我從上一輪的 `find` 結果推算的，不是從這條指令複製的。上一列不改寫，留著 | 我自己，在讀 commit `4915de3` 的輸出時 | 沒有新閘。這是觀測重點第 1 條在同一輪的第 2 次，且發生在**修正上一則錯誤的那次提交裡**——與 2026-09-22 CodeQL 紀錄「第四次發生在正在寫這條規則的那次提交裡」同形 |
...
- **有沒有在指令執行前就寫下結果。** **有，2 次**（原寫 1 次，第 2 次發生在寫完這一句之後）：第四節第 2 則，pytest 蒐集檢查那條指令裡預寫的 echo 結論；第四節第 5 則，修 Codex P1 那次 commit 裡把檔數寫成 42／39，實際輸出是 43／40。數字類（計數、SHA、exit code）本輪全部從輸出複製，兩份索引檔的每一個數字都能對回本 session 的指令輸出。
