...
本目錄的「外部」是相對本倉庫的取得位置，不是把權利人提供的 MRL 原件排除於 MRL。MRL 來源原件、鏡像、生成索引與第三方原材料须分開記錄。

上表 EXTERNAL_FORK_REFERENCES 的概括分類，以該檔 2026-09-26 更正為準：索引 artifact_owner 為 Mr.liou；關鍵字未命中不支持「MRL 貢獻 0%」。第三方具體材料的原聲明仍保留。DELTA 的未決對象為倉庫映射，不是 Mr.liou / MrLiouWord 的來源權位。

本目錄現已包含本倉產生的查核／修正文件，不能再以「這裡只存外部文件」概括所有內容。來源判定依各文件 source_artifact、derivative_role、transformation；保存位置不移轉任何來源權位。
